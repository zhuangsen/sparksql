...
object TopNStatJob2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val accessDF = spark.read.format("parquet").load("/home/madison/data/imooc/clean")
    ...
    try {
      videoAccessTopNDF.foreachPartition(partitionOfRecords => {
        ...
        partitionOfRecords.foreach(info => {
          val day = info.getAs[Integer]("day")
  ...
  def cityAccessTopNStat(spark: SparkSession, commonDF: DataFrame): Unit = {
    ...
    /**
      ...
      */
    //    accessDF.createOrReplaceTempView("access_logs")
    //    val cityAccessTopNDF = spark.sql("select day,cmsId,count(1) as times from access_logs " +
    //      "where day='20170511' and cmsType='video' " +
    //      "group by day,cmsId order by times desc")
